...
int main()
{
    ...
    double R0_f = 0.4, mu_f = 0.6, tau = 1.5;
    vector<double> rVec;
...
void Net2StrainSIR(double beta_f, double beta_s, double mu_f, double mu_s, double sigma, int NNodes, Vertex Nodes[])
{
    ...
    for (size_t i = 0; i < NNodes; i++)
    {
        ...
        if (Nodes[i].Status_old == 1 || Nodes[i].Status_old == 5)
        { // if Status == I_s or I_fs
            // transmition
            for (int v : Nodes[i].adjList)
            {
                double r1 = unifreal_dis(gen);
                if (Nodes[v].Status_old == 0 && r1 < beta_s)
                {                        // if Status==S
                    Nodes[v].Status = 1; //Status = I_s
                    Nodes[v].Infector = i;
                }
                if (Nodes[v].Status_old == 4 && r1 < pt_f_s)
                {                        // if Status==R_f
            ...
            if (Nodes[i].Status_old == 1 && r2 < mu_s)
            {                        // if Status == I_s
                Nodes[i].Status = 3; // Status = R_s
            }
            else if (Nodes[i].Status_old == 5 && r2 < mu_s)
            {                        // if Status == I_fs
                Nodes[i].Status = 7; // Status = R
            }
        }
        else if (Nodes[i].Status_old == 2 || Nodes[i].Status_old == 6)
        { // if Status == I_f or I_sf
            // transmition
            for (int v : Nodes[i].adjList)
            {
                double r1 = unifreal_dis(gen);
                if (Nodes[v].Status_old == 0 && r1 < beta_f)
                {                        // if Status==S
                    Nodes[v].Status = 2; //Status = I_f
                    Nodes[v].Infector = i;
                }
                if (Nodes[v].Status_old == 3 && r1 < pt_s_f)
                {                        // if Status==R_s
            ...
            // recovery
            double r2 = unifreal_dis(gen);
            if (Nodes[i].Status_old == 2 && r2 < mu_f)
            {                        // if Status == I_f
                Nodes[i].Status = 4; // Status = R_f
            }
            else if (Nodes[i].Status_old == 6 && r2 < mu_f)
            {                        // if Status == I_sf
                Nodes[i].Status = 7; // Status = R
            }
        }
